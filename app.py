...
# ==========================================
# 可视化图表绘制函数区 (全面英文化，规避字体问题)
# ==========================================
def plot_overall_radar(r_m1, r_m2, r_m3, r_m4):
    categories = ['Classroom Control', 'Mental Stimulation', 'Core Competencies', 'Eval Feedback']
    fig = go.Figure()
    ...
    fig.update_layout(
        ...
        title=dict(text="Normalized Scores (100-pt scale)", x=0.5, y=0.98, font=dict(size=14))
    )
    return fig
...
def plot_st_chart(sequence):
    ...
    ax.plot(t_counts, s_counts, marker='', linestyle='-', color='#1f77b4', linewidth=3, label="Actual Trajectory")
    if len(t_counts) > 0 and len(s_counts) > 0:
        ax.plot(t_counts[-1], s_counts[-1], marker='o', color='red', markersize=8, label="End Point")
    ...
    ax.set_xlabel("Teacher Action Time (T)", fontsize=10)
    ax.set_ylabel("Student Action Time (S)", fontsize=10)
    warning_text = "\n(Warning: Record too short)" if total_sec < 600 else ""
    ax.set_title(f"Classroom S-T Analysis\n(Sampling Interval: {interval}s){warning_text}", fontsize=12, fontweight='bold')
    ax.grid(True, linestyle='--', alpha=0.5)
    ax.plot([0, max_axis], [0, max_axis], linestyle='-', color='#FF9999', alpha=0.8, linewidth=1.5, label="Balance Line (45deg)")
...
def plot_bloom_pie(high, low):
    ...
    ax.pie(sizes, labels=['Higher-order', 'Lower-order'], colors=['#ff9999', '#66b3ff'], autopct='%1.1f%%', startangle=90)
    ax.axis('equal')
    ax.set_title("Bloom's Taxonomy", fontweight='bold')
    return fig

def plot_four_w_bar(know, app):
    fig, ax = plt.subplots(figsize=(4, 4))
    ax.bar(['Knowledge', 'Application'], [know, app], color=['#99ff99', '#ffcc99'])
    ax.set_ylabel("Question Count")
    ax.set_title("Question Types", fontweight='bold')
    return fig

def plot_m3_radar(metrics_m3):
    labels = ['Political', 'Scientific', 'Rule of Law', 'Public Part.']
    ...
    ax.set_title("Core Competencies Balance", y=1.1, fontweight='bold', fontsize=14)
...
def render_m3_altair_bar(metrics_m3):
    data = []
    dims = [
        ('Political', metrics_m3['political_endorsement']), 
        ('Scientific', metrics_m3['scientific_spirit']),
        ('Law', metrics_m3['rule_of_law']), 
        ('Public', metrics_m3['public_participation'])
    ]
    level_names = ['Level 1 (Red)', 'Level 2 (Yellow)', 'Level 3 (Blue)', 'Level 4 (Green)']
    colors = ['#FF4B4B', '#FACA2B', '#007BFF', '#28A745'] 
    
    for dim_name, metrics in dims:
        counts = metrics['counts']
        data.extend([
            {"Dimension": dim_name, "Level": level_names[0], "Count": counts['N1']},
            {"Dimension": dim_name, "Level": level_names[1], "Count": counts['N2']},
            {"Dimension": dim_name, "Level": level_names[2], "Count": counts['N3']},
            {"Dimension": dim_name, "Level": level_names[3], "Count": counts['N4']}
        ])
        
    df = pd.DataFrame(data)
    chart = alt.Chart(df).mark_bar(cornerRadiusTopLeft=3, cornerRadiusTopRight=3).encode(
        x=alt.X('Dimension:N', axis=alt.Axis(labelAngle=0, labelFontSize=12), title=None),
        y=alt.Y('Count:Q', title='Slice Count'),
        color=alt.Color('Level:N', scale=alt.Scale(domain=level_names, range=colors), legend=alt.Legend(title="Competency Level", orient="bottom")),
        tooltip=['Dimension', 'Level', 'Count']
                ...
                if metrics_m4:
                    ...
                    with c2:
                        fig1 = go.Figure(go.Pie(
                            ...
                            labels=["Answered", "Unanswered"], hole=0.6, marker_colors=["#4CAF50", "#E0E0E0"], textinfo='none'
                        ))
                    ...
                    with c4:
                        fig2 = go.Figure(go.Pie(
                            ...
                            labels=["Positive", "Neutral/Regular"], hole=0.6, marker_colors=["#FF9800", "#E0E0E0"], textinfo='none'
                        ))
